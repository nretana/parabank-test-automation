...
test("Successful signup using valid data", { tag:["@smoke", "@regression"] }, async ({ page }) => {
  const { firstName, lastName, address, city, state, zipCode, phone, ssn, username, password, confirm } = TEST_USER_REGISTRATION
    .valid[0] as UserRegistration;
  const signupPage = new SignUpPage(page);
  await signupPage.nagivate();
  await signupPage.submitSignUp(firstName, lastName, address, city, state, zipCode, phone, ssn, username, password, confirm);
  ...
  await expect(signupPage.userRegisteredHeading).toHaveText(signupPage.expectedUserRegisteredHeading(username));
